...
func (m MovieModel) Insert(movie *Movie) error {
	...
	//Prepare the arguments to be passed to the query
	//pq.Array converts the slice to a postgres array
...
func (m MovieModel) Get(id int64) (*Movie, error) {
	...
	//Movie struct to hold the result
	var movie Movie
...
func (m MovieModel) GetAll(title string, genres []string, filters Filters) ([]*Movie, Metadata, error) {
	...
	args := []any{title, pq.Array(genres), filters.PageSize, filters.offset()}
	...
	//put the scanned count from the window function into totalRecords
	totalRecords := 0

	//Slice to hold the results
	movies := []*Movie{}

	//Scan the rows into the slice
	for rows.Next() {
		var movie Movie
		err := rows.Scan(
			&totalRecords,
